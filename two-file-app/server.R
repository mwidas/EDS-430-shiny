...
server <- function(input, output) {
  ...
  trout_filtered_df <- reactive({
    ...
    validate(
      need(length(input$channel_type_input) > 0, "Please select at least 1 channel type to visualize data for."),
      need(length(input$section_input) > 0, "Please select at least 1 section (clear cut forest, or old growth forest) to visualize data for.")
  ...
  # alt text
  alt = "This is my alt text and it is not very good right now but we will improve later"
  )
  
  # filter penguin data by island ----
  island_df <- reactive({
    
    validate(
      need(length(input$island_input) > 0, "Please select at least 1 island to visualize data for."))
